...
layui.use('table', function () {
    ...
    tableIns = table.render({
        ...
        , cols: [[ //表头
            ...
            , {field: 'productName', title: '卡产品名称'}
            , {field: 'cardType', title: '卡类型'}
            , {field: 'binStart', title: '卡BIN开始'}
            , {field: 'binEnd', title: '卡BIN结束'}
            , {field: 'binDesc', title: '描述'}
            , {field: 'operator', title: '操作', width: 190, toolbar: '#barDemo', fixed: 'right'}
        ]],
        done: function (res, curr, count) {

            // 类型
            $("[data-field='cardType']").children().each(function () {
                if ($(this).text() === '1') {
                    $(this).text('信用卡')
                } else if ($(this).text() === '2') {
                    $(this).text("借记卡")
    ...
    var $ = layui.$, active = {
        reload: function () {
            ...
            var cardType = $('#search_cardType').val();
